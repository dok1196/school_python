...
def everything_for_your_cat(cats_data):
    ...
    for cat in cats_data:
        name = cat[2] + ' ' + cat[3]
        if name not in owners_dict:
            owners_dict[name] = [(cat[0], cat[1])]
        else:
            owners_dict[name].append((cat[0], cat[1]))
    our_str = ''
    for owner, cats in owners_dict.items():
        cat_str = ''
        for cat in cats:
            cat_str += cat[0] + ', ' + str(cat[1]) + '; '
        our_str += owner + ': ' + cat_str[:-2] + ' \n'
...
# Алексей Березуев: Гарфилд, 3;
# ''',
#
# '''
# Алексей Егоров: Мартин, 5; Мурзик, 1; Гарфилд, 3;
# Анна Самохина: Фродо, 3; Буся, 12; Рыжик, 7;
# Андрей Белов: Вася, 4; Матильда, 8; Марта, 5; Джонни, 10; Барсик, 2;
