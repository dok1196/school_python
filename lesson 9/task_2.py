...
def func_log(file_log='log.txt'):
    ...
    Аргументы:
    - `file_log` (str, optional): Путь до файла лога. По умолчанию равен 'log.txt'.
    Возвращаемое значение:
    - `decorator` (function): Внутренняя функция-декоратор, которая принимает
     функцию `func` и возвращает функцию-обертку `wrapper`.
    """
    def decorator(func):
        def wrapper(*args, **kwargs):
            now = datetime.datetime.now()
            with open(file_log, 'a') as f:
                f.write(f"{func.__name__} вызвана {now.strftime('%d.%m.%Y %H:%M:%S')}\n")
